...
static int ftv_remote_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
     ...
     */
	pr_info("%s: hdev->name = %s, vendor_id = %d, product_id = %d, version = %d, driver_data = %d\n",
		__func__, hdev->name, hdev->vendor, hdev->product, hdev->version, id->driver_data);
	...
	pr_info("%s: vendor_id = %d, product_id = %d, version = %d, driver_data = %d\n",
		__func__, remote_drvdata->input->id.vendor, remote_drvdata->input->id.product, remote_drvdata->input->id.version, id->driver_data);
...
static const struct hid_device_id ftv_remote_devices[] = {
	...
	{HID_BLUETOOTH_DEVICE(BT_VENDOR_ID_LAB126, BT_DEVICE_ID_LAB126_WILBUR)},
	{HID_USB_DEVICE(BT_VENDOR_ID_LAB126, BT_DEVICE_ID_LAB126_WILBUR)},
