...
static void _CEC_Receiving(struct mtk_cec *cec)
{
	...
	if (true != CEC_rx_enqueue(frame)) {
		SetCECErrorFlag(ERR_RXQ_OVERFLOW);
		TX_DEF_LOG("[CEC] cec rx queue full\n");

		/* workaround solution */
		TX_DEF_LOG("[CEC] Tx fsm=0x%lx\n",
			(cec->cec_read(cec, CEC2_TX_STATUS) & CEC2_TX_FSM) >> 16);
		TX_DEF_LOG("[CEC] Rx fsm=0x%lx\n",
			(cec->cec_read(cec, CEC2_RX_STATUS) & CEC2_RX_FSM) >> 16);
		/* clear Rx queue*/
		memset(CEC_rx_msg_queue, 0, sizeof(CEC_FRAME_DESCRIPTION_IO) * RX_Q_SIZE);
		CEC_rxQ_write_idx = 0;
		CEC_rxQ_read_idx = 0;

		mtkcec_hwrx_reset(cec);
		TX_DEF_LOG("[CEC] Rx queue full:hw reset Rx & Tx\n");
		mtkcec_hwtx_reset(cec);

		/* clear all cec tx interrupt */
		mtkcec_hwtx_int_alldisable(cec);
		mtkcec_hwtx_int_all_clear(cec);

		ClrCECStatus(STATE_WAIT_TX_DATA_TAKEN |	STATE_TX_NOACK |
			STATE_TXING_FRAME | STATE_TX_FRAME_SUCCESS |
			STATE_HW_RETX |	STATE_WAIT_RX_FRAME_COMPLETE |
			STATE_RX_COMPLETE_NEW_FRAME | STATE_HW_RX_OVERFLOW |
			STATE_RX_GET_NEW_HEADER | STATE_WAIT_TX_CHECK_RESULT |
			STATE_TXFAIL_DNAK | STATE_TXFAIL_HNAK | STATE_TXFAIL_RETR|
			STATE_TXFAIL_DATA |	STATE_TXFAIL_HEAD | STATE_TXFAIL_SRC |
			STATE_TXFAIL_LOW);
		ClrCECErrorFlag(ERR_TX_BUFFER_LOW | ERR_TX_UNDERRUN |
			ERR_TX_MISALARM | ERR_RX_LOST_EOM |
			ERR_RXQ_OVERFLOW | ERR_RX_LOST_HEADER);
		/* workaround solution */
...
static unsigned char _CEC_SendFrame(struct mtk_cec *cec,	CEC_FRAME_DESCRIPTION_IO *frame)
{
	...
	if (IsCECStatus(STATE_TXING_FRAME | STATE_WAIT_TX_CHECK_RESULT)) {
		HDMI_CEC_COMMAND_LOG("TX FAIL in %s,line=%d\n", __func__, __LINE__);
		return 1;
	}
	/* CYJ.NOTE: Leave "TX hardware error handling" to _CEC_Mainloop */
	if (mtkcec_hwtx_fail_fsm(cec) || mtkcec_hwtx_fail(cec)) {
		HDMI_CEC_COMMAND_LOG("Detect TX FAIL in %s,line=%d\n", __func__, __LINE__);
		mtkcec_hwtx_reset(cec);
...
unsigned char _CEC_TX_Enqueue(CEC_FRAME_DESCRIPTION_IO *frame)
{
	...
	if (IS_TX_Q_FULL()) {
		TX_DEF_LOG("[CEC] Tx queue is full\n");
		...
		/* clear Tx queue*/
		memset(CEC_tx_msg_queue, 0, sizeof(CEC_FRAME_DESCRIPTION_IO) * TX_Q_SIZE);
		CEC_txQ_write_idx = 0;
		CEC_txQ_read_idx = 0;

		mtkcec_hwrx_reset(cec);
		TX_DEF_LOG("[CEC] Tx queue full:hw reset Rx & Tx\n");
		mtkcec_hwtx_reset(cec);
...
static unsigned int cec_loop_count;
void hdmi_cec_mainloop(unsigned char u1rxmode)
{
	...
	if (cec_loop_count >= 500) { //dump status every 10s
		cec_loop_count = 0;
		TX_DEF_LOG("[CEC] %s\n", __func__);
		TX_DEF_LOG("[CEC] _CEC_Status=0x%x\n", _CEC_Status);
		TX_DEF_LOG("[CEC] _CEC_ErrStatus=0x%x\n", _CEC_ErrStatus);
		TX_DEF_LOG("[CEC] Tx fsm=0x%lx\n",
			(cec->cec_read(cec, CEC2_TX_STATUS) & CEC2_TX_FSM) >> 16);
		TX_DEF_LOG("[CEC] Rx fsm=0x%lx\n",
			(cec->cec_read(cec, CEC2_RX_STATUS) & CEC2_RX_FSM) >> 16);
...
unsigned char hdmi_cec_isrprocess_new(struct mtk_cec *cec, unsigned char u1rxmode)
{
	...
	if (IsCECStatus(STATE_TXING_FRAME)) {
		if (mtkcec_hwtx_fail_longlow(cec)) {
			...
			mtkcec_hwtx_fail_longlow_intclr(cec);
			TX_DEF_LOG("[CEC] long low interrupt!!\n");
...
void hdmi_CECMWSend(CEC_SEND_MSG *msg)
{
	...
	if ((cec->txmsg_usr.t_frame_info.ui1_init_addr > 0xf)
		...
		cec->txmsg_usr.result.pv_tag = cec->txmsg_usr.pv_tag;
		cec->txmsg_usr.result.e_ack_cond = APK_CEC_ACK_COND_NO_RESPONSE;
		TX_DEF_LOG("[CEC] apk send msg error\n");
		goto done;
	}
...
void hdmi_CECMWSetEnableCEC(unsigned char u1EnCec)
{
	...
	if ((u1EnCec == 1) && (hdmi_cec_on == 0)) {
		HDMI_CEC_COMMAND_LOG("UI ON\n");
		hdmi_cec_on = 1;
		hdmi_cec_power_on(cec, true);
		hdmi_cec_init(cec);
		cec_timer_wakeup();
	} else if ((u1EnCec == 0) && (hdmi_cec_on == 1)) {
		HDMI_CEC_COMMAND_LOG("UI off\n");
		hdmi_cec_on = 0;
		cec_timer_sleep();
		hdmi_cec_deinit(cec);
		hdmi_cec_power_on(cec, false);
	} else {
		HDMI_CEC_COMMAND_LOG("[hdmi_cec]on/off fail,en=%d,hdmi_cec_on=%d\n", u1EnCec, hdmi_cec_on);
	}
}
