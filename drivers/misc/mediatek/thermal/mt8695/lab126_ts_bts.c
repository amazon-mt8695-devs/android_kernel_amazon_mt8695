...
#define Proto_abc123_BOARD_REV (0x00)
#define HVT_abc123_BOARD_REV (0x10)
#define EVT_abc123_BOARD_REV (0x20)
#define DVT_abc123_BOARD_REV (0x30)
#define PVT_abc123_BOARD_REV (0x40)
#endif /* CONFIG_IDME */
...
static int ntc_bts_probe(struct platform_device *pdev)
{
	...
	if (board_rev < 0 || board_rev > PVT_abc123_BOARD_REV) {
		pr_err("%s: board_rev:0x%x out of range\n", __func__, board_rev);
	}
	if (board_rev >= 0 && board_rev < HVT_abc123_BOARD_REV ) {
		bts_channel_param[serial].g_RAP_pull_up_R = PRIOR_HVT_PULLUP_R;
	}
	pr_info("bts device adjusted pullup R: %d\n", bts_channel_param[serial].g_RAP_pull_up_R);
