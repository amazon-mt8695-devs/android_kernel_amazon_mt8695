...
static long tz_client_open_session(struct file *file, unsigned long arg)
{
	...
	struct kree_session_tag_cmd_param param = {0, 0, 0, 0, 0};
			...
			u8 uparam[REE_SERVICE_BUFFER_SIZE])
{
	...
	secure_pages = cma_alloc(tz_cma, secure_size / SZ_4K, get_order(SZ_1M));
	if (secure_pages == NULL) {
...
TZ_RESULT KREE_ServReleaseChunkmemPool(u32 op,
			u8 uparam[REE_SERVICE_BUFFER_SIZE])
{
	phys_addr_t secure_phy = page_to_phys(secure_pages);

	if (secure_pages != NULL) {
		cma_release(tz_cma, secure_pages,
				cma_get_size(tz_cma)>>PAGE_SHIFT);
		pr_warn("%s() release @%pap [0x%zx]\n", __func__,
				&secure_phy, secure_size);
		secure_pages = NULL;
		secure_size = 0;
		return TZ_RESULT_SUCCESS;
	}
...
{
	do {
		if (wait_for_completion_interruptible(&tz_cm_shrinker_work) != 0)
			continue;
...
#ifdef CONFIG_MTK_IN_HOUSE_TEE_CRYPTO_DRIVER
...
static int __init rmem_tz_sec_setup(struct reserved_mem *rmem)
{
	...
	pr_err("[MTEE] rmem_tz_sec_setup base is %pap,size is  %pap\n", &rmem->base, &rmem->size);
	if (!of_get_flat_dt_prop(node, "reusable", NULL) || of_get_flat_dt_prop(node, "no-map", NULL)) {
		pr_err("[MTEE] rmem_tz_sec_setup error\n");
		return -EINVAL;
	}
...
static int __init tz_init(void)
{
	...
	struct device_node *mtee_node;

	mtee_node = of_find_node_by_path("/mtee");
	if (mtee_node == NULL)
		pr_warn("can't find mtee device node from device tree\n");

	parent_node = of_irq_find_parent(mtee_node);
	if (parent_node)
