			    ...
			    ion_phys_addr_t *addr, size_t *len)
{
	...
	port_info.pretmvauf = &buffer_info->MVA;
	...
	if (((*(unsigned int *)addr & 0xffff) == ION_FLAG_GET_FIXED_PHYS) &&
		...
		port_info.pretmvauf = &buffer_info->FIXED_MVA;
	}
	...
	if (((buffer_info->MVA == 0) && (port_info.flags == 0)) ||
		...
		*(unsigned int *)addr = *(unsigned int *)(port_info.pretmvauf);
		...
		for (n = rb_first(&dev->buffers); n; n = rb_next(n)) {
			...
			*buffer = rb_entry(n, struct ion_buffer, node);
			...
			ION_PRINT_LOG_OR_SEQ(s,
					     ...
					     buffer->handle_count, bug_info->module_id, bug_info->MVA,
					     bug_info->security, buffer->flags, buffer->pid, buffer->task_comm,
					     pdbg->value1, pdbg->value2, pdbg->value3, pdbg->value4, pdbg->dbg_name);
...
struct ion_heap *ion_mm_heap_create(struct ion_platform_heap *unused)
{
	...
	heap->heap.type = (enum ion_heap_type)ION_HEAP_TYPE_MULTIMEDIA;
...
/*
 * tablet CMA buffer is small than phone's implement, and due to tablet's hybrid
 * enc HW will always use security buffer which allocated from CMA buffer. But camera
 * heap will keep CMA buffer in normal world and cause enc could not get the security
 * buffer. Tablet do not want the CMA buffer be kept in normal world in such case.
 *
 * Let camera allocate from DMA buffer instead of CMA buffer for tablet use.
 * CONFIG_MTK_IN_HOUSE_TEE_SUPPORT was only be enabled for tablet platforms.
 */
...
void shrink_ion_by_scenario(void)
{
	...
	do {
		/*__GFP_CMA build error for mt8695*/
