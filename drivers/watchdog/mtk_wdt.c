...
#ifdef CONFIG_MACH_LEOPARD
	/* Reset MCU of N9 before AP reset until memory init done after platform reboot */
	reg = ioread32(wdt_base + WDT_SWSYSRST);
	reg = WDT_SWSYSRST_KEY | WDT_CONN_MCU_RST;
	iowrite32(reg, wdt_base + WDT_SWSYSRST);
	pr_info("WDT_SWSYSRST: 0X%X\n", ioread32(wdt_base + WDT_SWSYSRST));
#endif
...
static void wdt_report_info(void)
{
	...
	task = &init_task;
	pr_err("Qwdt: -- watchdog time out\n");

	for_each_process(task) {
		if (task->state == 0) {
			pr_err("PID: %d, name: %s, on_cpu: %d\n backtrace:\n", task->pid, task->comm, task->on_cpu);
			show_stack(task, NULL);
			pr_debug("\n");
		}
	}

	pr_err("backtrace of current task:\n");
